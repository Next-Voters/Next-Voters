...
// Always redirect post-auth users to /subscription. /subscription is
// cookie-aware and handles every flow:
//   - subscribe-intent cookie -> fires Stripe kickoff
//   - request-intent cookie  -> bounces to /subscription/onboarding (wizard
//                               auto-submits the waitlist)
//   - existing subscriber    -> SubscriptionDashboard
//   - fresh user, no cookie  -> bounces to /subscription/onboarding (step 1)
//
...
// Hard-coding /subscription removes that fragile coupling entirely.
export async function GET(request: NextRequest) {
  ...
  if (exchangeFailed) {
    return NextResponse.redirect(`${origin}/subscription/onboarding?error=oauth_failed`);
  }
